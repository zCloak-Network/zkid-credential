...
function CTypeCard({ actions, ctype, meta }: Props) {
  ...
  return (
    ...
    >
      <IconButton onClick={handleDelete} size='small' sx={{ position: 'absolute', right: 10, top: 10 }}>
        <IconDelete />
      </IconButton>
      <Stack spacing={1.5}>
        <Box className='CTypeCard_logo'>
          {meta?.icon ? <Box component='img' src={meta.icon} sx={{ width: 50, height: 50 }} /> : <IconLogoCircle />}
        </Box>
